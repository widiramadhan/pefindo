<?php
if(isset($array['GetCustomReportResponse']['GetCustomReportResult']['aCIP']['bRecordList']['bRecord'])){
	foreach($array['GetCustomReportResponse']['GetCustomReportResult']['aCIP']['bRecordList']['bRecord'] as $item){
		...
		$callCIP = "{call SP_INSERT_CIP_COMPANY(?,?,?,?,?,?)}";
		$paramsCIP =array(
	...
	/* Reason List */
	$callGetCIP = "{call SP_GET_ID_M_CIP_COMPANY(?)}";
	...
	if(isset($item['bReasonsList']['bReason'])){
		foreach($item['bReasonsList']['bReason'] as $data){
			...
			$callReasonList = "{call SP_INSERT_CIP_REASONS_LIST_COMPANY(?,?,?,?)}";
